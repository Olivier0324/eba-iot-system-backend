...
export const generatePDF = async (data, options) => {
    ...
    const doc = new PDFDocument({
        ...
        size: 'A4',
        info: {
            Title: 'EBA SYTEM REPORT',
            Author: 'EBA System',
            Producer: 'ECOBASED ADPTATION SYSTEM PDF GENERATOR'
        }
    });
    ...
    if (data.length > 0) {
        ...
        data.forEach((d) => {
            ...
            doc.fillColor(tempColor)
                .text(`${d.temperature.toFixed(1)}°C`, 150, y);

            doc.fillColor(styles.tableRow.color)
                .text(`${d.humidity.toFixed(1)}%`, 240, y)
                .text(`${d.co2_ppm}`, 330, y)
    ...
    if (data.length > 0) {
        ...
        y = y + 300; // Update y position after statistics
    }
    ...
    // Instead of using switchToPage, we'll add footer to each page as we create them
    // For the current approach, let's add footer to the last page only

    // Add footer to current page
    const addFooter = () => {
        // We need to know page numbers, but PDFKit doesn't provide easy page counting during generation
        // Simple approach: just add a generic footer
